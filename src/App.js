...
function Usersignup()
{
  ...
  return (
    <div>
      ...
      <button type="submit" onClick={()=>signUp(userdata).then((x)=>(x===200)&&history.push('/userlogin'))}>Submit</button>
      {/* <button type="submit" onClick={()=>history.push('/userlogin')}>Login</button> */}
...
function Userlogin()
{
  ...
  return (<div>
    <input type="text" onInput={(e)=>setMailid(e.target.value)} placeholder="Enter The Mailid" />
      ...
      <button type="submit" onClick={()=>history.push('/Employeelogin')}>Employee Portal</button>
  </div>)
}
