...
function Employeelogin() {
  const { Firstname,Lastname, Mailid, setMailid, Password, setPassword, history } = useContext(context);
  ...
  {
        return history.push('/Managerdashboard')
  }
  else if(userdata.Designation==="Employee")
  {
        return history.push('/Employeedashboard')
  }
}
  return (<div>
    ...
    {/* <button type="submit" onClick={() => history.push('/Employeesignup')}>Signup</button> */}
    <button type="submit" onClick={() => logIn(userdata).then((x) => (x === 200)?page():null)}>Login</button>
    <button type="submit" onClick={() => history.push('/employeeforgotpassword')}>Forgot Password</button>
  </div>);
}
 ...
 function Employeesignup() {
  ...
  return (<div>
    ...
    <input type="text" onInput={(e) => setPassword(e.target.value)} placeholder="Enter The Password" />
    <button type="submit" onClick={() => signUp(userdata).then((x) => (x === 200) && history.push('/Employeelogin'))}>Get Started</button>
  </div>);
}
...
//   console.log( forgotresponse.status);
  ...
  return(<div>
    <input type="text" onInput={(e)=>setMailid(e.target.value)} placeholder="Enter The Mailid" />
    <button type="submit" onClick={()=>{forgot(userdata).then((x)=>(x===200)&&history.push('/employeeupdatepassword'))}}>Submit</button>
  </div>)
}
...
{/*  
   <button type="submit" onClick={() => history.push('/Employeesignup')}>Add Employee</button>
   <button type="submit" onClick={() => history.push('/adduser')}>Add User</button> */}
